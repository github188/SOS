...
* ##### 工具辅助脚本：
  ...
  被测试设备总部内网网段（如：111.111.111.0）：    ------>测试总部的内网IP，不支持多总部多个内网网段（有替代办法）
...
```
  常见问题：
      1.VPN隧道建立成功后，业务访问不通，在总部vpntun抓不到回应包？
      总部内网网段配置错误（只能手动设置，且只能手动设置一个网段），若需要添加别的网段，需要修改文件/etc/sinfor/dlan/RouteSet.vpn
  定位手段：
